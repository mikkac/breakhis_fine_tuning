...
def load_test_data(test_csv, image_processor):
    ...
    dataset = dataset.map(lambda e: process_dataset(e, image_processor), with_indices=False, num_proc=2)
...
def main_evaluation(model_id, checkpoint_path, test_csv, batch_size, cpu_threads):
    ...
    metrics = evaluate_model(model, test_dataset, batch_size, cpu_threads)
    print(metrics)
